...
const Contact = () => {
  ...
  const handleEmailChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const fromEmailField = form.current?.querySelector(
  ...
  const handleNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const userNameField = form.current?.querySelector(
  ...
  return (
    <section
      id="contact"
      className="py-20 bg-gradient-to-t from-indigo-100 via-pink-50 to-white"
    >
      <div className="max-w-xl mx-auto px-6">
        ...
        <form
          ref={form}
          onSubmit={sendEmail}
          className="bg-white rounded-2xl p-8 shadow-xl fade-in"
        >
          <div className="mb-4">
          ...
          <div className="mb-6">
            ...
            <textarea
              ...
              id="message"
              rows={4}
              placeholder="Estas interesado en alguna pintura? Preguntame aqui!"
              required
              aria-label="Mensaje"
            />
          </div>
          <button
            type="submit"
            className="w-full py-3 bg-pink-500 hover:bg-indigo-500 text-white font-bold rounded-lg transition"
